...
var auth = function (req, res, next) {
    ...
    if (token) {
        ...
        jwt.verify(token, "93%&*#IRY5FHMN664357bnbhjff#()90", function (err, decoded) {
...
var transporter = nodemailer.createTransport({
    ...
    var mailOptions = {
        from: 'WeBest Support <support@WeBestSupport.com>',
        to: data.email,
        subject: 'WeBest Support Notifications',
        text: 'Answer received for ticket number : ' + data.ticketNumber,
        html: '<h1>Hello ' + data.name + '</h1><br><h2>This is a initimation from WeBest Support.</b>Someone replied for ticket number : ' + data.ticketNumber + '</h2>'
    }
        ...
        if (error) {
            ...
            result.save(function (error) {
                if (error) {
                    ...
                    var response = resGenerator.generate(false, "Status changed successfully to : " + result.ticketStatus, 200, result);
        ...
        result.message.push(newMessage);
            result.save(function (error) {
                if (error) {
                    ...
                    var response = resGenerator.generate(false, "A new Message for ticket created successfully", 200, result);
                    res.send(response);
                }
            });
        }
        ...
        if (error) {
            ...
            newTicket.save(function (error) {
                if (error) {
                    ...
                    var response = resGenerator.generate(false, "Ticket inserted successfully", 200, shortid.generate());
                    res.send(response);
                }
            });
